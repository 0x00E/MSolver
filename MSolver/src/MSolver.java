...
public class MSolver{
  ...
  static void calibrate(){
	System.out.println("MSolver 2018年3月8日修改版 MIT LICENSE By LuckyToilet");
    // Display this message
    System.out.println("扫描屏幕...");
    ...
    System.out.printf("横向方块=%d, 纵向方块=%d, 方块间距=%f\n", BoardWidth, BoardHeight, BoardPix);
    System.out.printf("面板X坐标=%d, 面板Y坐标=%d\n",BoardTopW, BoardTopH);
  ...
  static int detect(BufferedImage bi, int i, int j){
    ...
    for(int rgb : areapix){
      ...
      System.out.println(colorDifference(red, green, blue, 110,110,110));
      // Detect death
  ...
  static void attemptMove() throws Throwable{
    ...
    if(success) {
    	System.out.println("计算成功，跳过坦克算法！");
    	return;
    }else {
    	System.out.println("无法计算，进入坦克算法！");
    }
  ...
  static void guessRandomly() throws Throwable{
    System.out.println("无法得出，根据几率随机点击！");
    while(true){
  ...
  static void tankSolver() throws Throwable{
    ...
    ArrayList<Pair<Integer,Integer>> borderTiles = new ArrayList<Pair<Integer,Integer>>();
    ArrayList<Pair<Integer,Integer>> allEmptyTiles = new ArrayList<Pair<Integer,Integer>>();
    ...
    for(int i=0; i<BoardHeight; i++)
      for(int j=0; j<BoardWidth; j++)
        if(onScreen(i,j) == -1 && !flags[i][j])
          allEmptyTiles.add(new Pair<Integer, Integer>(i,j));

    // Determine all border tiles
    for(int i=0; i<BoardHeight; i++)
      for(int j=0; j<BoardWidth; j++)
        if(isBoundry(onScreen,i,j) && !flags[i][j])
          borderTiles.add(new Pair<Integer, Integer>(i,j));
    ...
    ArrayList<ArrayList<Pair<Integer,Integer>>> segregated;
    if(!borderOptimization){
      segregated = new ArrayList<ArrayList<Pair<Integer,Integer>>>();
    ...
    System.out.printf(
        "坦克算法随机几率 %1.2f，步骤 %d (%dms, %d 过程)%s\n",
        prob_best, numMines, tankTime, totalMultCases,
        (borderOptimization?"":"*"));
    Pair<Integer, Integer> q;
	try {
		q = segregated.get(prob_best_s).get(prob_besttile);
		int qi = q.getFirst();
	    int qj = q.getSecond();
	    clickOn(qi,qj);
  ...
  static ArrayList<ArrayList<Pair<Integer, Integer>>>
            tankSegregate(ArrayList<Pair<Integer,Integer>> borderTiles){

    ArrayList<ArrayList<Pair<Integer,Integer>>> allRegions = new ArrayList<ArrayList<Pair<Integer,Integer>>>();
    ArrayList<Pair<Integer,Integer>> covered = new ArrayList<Pair<Integer,Integer>>();

    while(true){

      LinkedList<Pair<Integer,Integer>> queue = new LinkedList<Pair<Integer,Integer>>();
      ArrayList<Pair<Integer,Integer>> finishedRegion = new ArrayList<Pair<Integer,Integer>>();

      // Find a suitable starting point
      for(Pair<Integer,Integer>firstT : borderTiles){
  ...
  static void tankRecurse(ArrayList<Pair<Integer, Integer>> arrayList, int k){
    ...
    if(k == arrayList.size()){
      ...
      boolean[] solution = new boolean[arrayList.size()];
      for(int i=0; i<arrayList.size(); i++){
        Pair<Integer,Integer> s = arrayList.get(i);
        int si = s.getFirst();
        int sj = s.getSecond();
    ...
    Pair<Integer,Integer> q = arrayList.get(k);
    int qi = q.getFirst();
    int qj = q.getSecond();

    // Recurse two positions: mine and no mine
    knownMine[qi][qj] = true;
    tankRecurse(arrayList, k+1);
  ...
  public static void main(String[] args) throws Throwable {
    ...
    */
    ...
    while(true) {
      ...
      if(status == -10) {
    	  System.out.println("游戏结束(胜利/失败)，程序结束。");
    	  exit();
      }
      attemptFlagMine();
      attemptMove();
